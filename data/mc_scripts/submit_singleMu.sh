...
#SBATCH -n 300   #Total number of tasks
#SBATCH --tasks-per-node=25
#SBATCH --image=ethlu/sl7_dune:a
#SBATCH -q regular -t 3:30:00 #Max hours: regular,premium=48 hrs; debug=0.5 hrs
#SBATCH -A dune

cd /global/project/projectdirs/dune/users/cjslin/protodune/mc_scripts/
export my_output=mcProduction.log

echo 'Production starting at  '`date` > ${my_output}

srun --kill-on-bad-exit=0 --no-kill --label shifter -- ./gen_fullchain_singleMu.sh >> ${my_output}
