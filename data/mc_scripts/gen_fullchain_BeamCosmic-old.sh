...
#BASEDIR=${SCRATCH}/larsim/pdsp_chnoiseAug2018
#TAG=reco_1GeV
TAG=reco_1GeV-pdsp_chnoiseAug2018

cd ${BASEDIR}/BeamCosmic/${TAG}

#Create temporary data output directory
mkdir ${JOBID}
cd ${JOBID}
cp /global/homes/e/ethanlu/mlreco_cell/data/mc_scripts/* .

DATAOUT=./
DATAOUT2=../
LOGOUT=${BASEDIR}/logs/${TAG}-BeamCosmic_${JOBID}.log
...
G4FILE=g4_BeamCosmic_${JOBID}.root
DETSIMFILE=detsim_${JOBID}.root
RECOFILE=reco_BeamCosmic_${JOBID}.root
